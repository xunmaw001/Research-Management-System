...
export default {
	data() {
		...
		return {
			...
			rules: {
				duiwuzhanghao: [
					{ required: true, message: '队伍账号不能为空', trigger: 'blur' },
				],
				mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `keyanduiwu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.keyanduiwuCrossAddOrUpdateFlag = false;
						 this.parent.search();
