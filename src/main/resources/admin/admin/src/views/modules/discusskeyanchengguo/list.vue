		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"90%","margin":"30px auto 20px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20220726/bd28bed57c07435ab63ec9a9b5eb4c5c.png) no-repeat center top / 100% 100%","width":"auto","fontSize":"14px","height":"44px"}' v-if="isAuth('discusskeyanchengguo','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20220726/bd28bed57c07435ab63ec9a9b5eb4c5c.png) no-repeat center top / 100% 100%","width":"auto","fontSize":"14px","height":"44px"}' v-if="isAuth('discusskeyanchengguo','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"5px solid #3FA8C4","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(63, 168, 196, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('discusskeyanchengguo','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>


							<el-button :style='{"border":"5px solid #3FA8C4","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(63, 168, 196, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('discusskeyanchengguo','查看评论')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">查看评论</el-button>
...
export default {
  ...
  methods: {
	...
	this.$router.push({path:'/discussdiscusskeyanchengguo',query:{refid:id}});
    },
    // 下载
    download(file){
      window.open(`${file}`)
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "discusskeyanchengguo/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
