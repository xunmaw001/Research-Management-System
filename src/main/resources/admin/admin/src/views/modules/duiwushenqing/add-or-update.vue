	...
	<div class="addEdit-block" :style='{"padding":"0"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="队伍名称" prop="duiwumingcheng">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="upload" v-if="type!='info'&& !ro.shenqingshu" label="申请书" prop="shenqingshu">
					<file-upload
						tip="点击上传申请书"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.shenqingshu?ruleForm.shenqingshu:''"
						@change="shenqingshuUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="ruleForm.shenqingshu" label="申请书" prop="shenqingshu">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"rgba(121, 121, 121, 1)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.shenqingshu)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="!ruleForm.shenqingshu" label="申请书" prop="shenqingshu">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="textarea" v-if="type!='info'" label="申请原因" prop="shenqingyuanyin">
					<el-input
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="ruleForm.shenqingyuanyin" label="申请原因" prop="shenqingyuanyin">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.shenqingyuanyin}}</span>
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='shenqingshu'){
							this.ruleForm.shenqingshu = obj[o];
							this.ro.shenqingshu = true;
							continue;
						}
	...
	if(this.ruleForm.shenqingshu!=null) {
		this.ruleForm.shenqingshu = this.ruleForm.shenqingshu.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `duiwushenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.duiwushenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shenqingshuUploadChange(fileUrls) {
	    this.ruleForm.shenqingshu = fileUrls;
    },
  }
};
</script>
